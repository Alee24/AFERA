...
export default function VirtualRoom() {
  ...
  const handleLaunch = (e: React.FormEvent) => {
    ...
    showNotification('Virtual session launched!', 'success');
  };
    ...
    <div className="space-y-10 max-w-6xl mx-auto">
      <div>
         <h1 className="text-4xl font-black text-primary dark:text-white tracking-tight">
            Virtual <span className="text-accent italic">Classroom Room</span>
         </h1>
         <p className="text-gray-500 mt-2 font-medium">Coordinate, interact, and stream fully immersive modules via Jitsi Meet integration.</p>
      </div>
      ...
      {!activeRoom ? (
         <div className="bg-white dark:bg-slate-900 p-10 rounded-[40px] shadow-lg border border-gray-100 dark:border-slate-800 flex flex-col md:flex-row items-center gap-10">
            <div className="flex-1 space-y-6">
               ...
               <div>
                  <h3 className="text-2xl font-bold text-primary dark:text-white">Create or Join a Live Module</h3>
                  <p className="text-sm text-gray-400 mt-1">Specify an identifier below to launch custom video feeds securely.</p>
               </div>

               <form onSubmit={handleLaunch} className="space-y-4">
                  <div className="space-y-2">
                     <label className="text-xs font-black uppercase tracking-widest text-gray-400">Classroom Identifier</label>
                     <input 
                       type="text"
                       value={roomName}
                       onChange={(e) => setRoomName(e.target.value)}
                       placeholder="e.g. Advanced AI Foundations"
                  ...
                  <Button type="submit" className="w-full h-16 bg-primary text-white rounded-2xl font-black uppercase tracking-widest shadow-xl shadow-primary/20">
            ...
            <div className="hidden md:block w-72 h-72 bg-gray-50 dark:bg-slate-800 rounded-[40px] relative overflow-hidden border border-gray-100 dark:border-slate-800">
               <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-8 text-gray-400">
                  <ShieldAlert size={48} className="mb-4 text-primary/30" />
                  <span className="text-xs font-bold uppercase tracking-widest text-primary/50">Jitsi Integration Ready</span>
               </div>
            </div>
         </div>
      ) : (
         <div className="space-y-6">
            <div className="flex items-center justify-between bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-800">
               <div className="flex items-center space-x-4">
                  ...
                  <div>
                     <h4 className="font-bold text-primary dark:text-white">Broadcasting Live</h4>
            ...
            <div className="w-full h-[70vh] rounded-[40px] overflow-hidden shadow-2xl border border-primary/20 bg-black">
               <iframe 
                 src={`https://meet.ffmuc.net/${activeRoom}#userInfo.displayName="${encodeURIComponent(user?.first_name || 'Lecturer')}"`} 
